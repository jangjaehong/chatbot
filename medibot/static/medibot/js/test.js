...
function dataRender(data, flag){
    ...
    for(var i=0; i < intakeData.length; i++){
        dataValue[i+1] =  [standardsData[i], intakeData[i]];
    }
...
function drawCircleChart(dataSet, container, flag){
    var containerRect = d3.select(container).node().getBoundingClientRect()
        containerWidth = containerRect.width - 80,
        containerHeight = containerRect.height- 80;

    var padding = {top:20, right:20, bottom:20, left:20},
        svgWidth = (containerWidth/3),
    ...
    for(var i=0; i < dataSet.length; i++){
        var svg = chart_container.append("svg")
            .attr("id", "fillgauge" + i)
            .attr("class","area fillgauge" + i)
            .attr("width", svgWidth + padding.right + padding.left)
        ...
        var gauge = loadLiquidFillGauge("fillgauge" + i, dataSet[i], config);
    }
// 원 그래프 그리기
...
function loadLiquidFillGauge(elementId, value, config) {
    ...
    var radius = Math.min(parseInt(gauge.style("width"))- 35, parseInt(gauge.style("height")) - 30)/2;
